...
A local, single-user Google Play research tool. It saves category charts, keyword searches, app details, review samples, and manual shortlist notes in SQLite. The browser never receives the SerpApi key.
...
Chart results remain fresh locally for 24 hours. Keyword searches and app details remain fresh for 7 days. A fresh match is returned without a provider call. Reviews are fetched only on request; the default is one page of up to 199 one-star reviews, with at most three pages per action. A preview shows the maximum possible searches before every fetch. `Refresh` bypasses the local cache; `Force live` also sets SerpApi `no_cache=true`. SerpApi may serve a refresh from its own one-hour cache for free.
